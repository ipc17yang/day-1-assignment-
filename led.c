...
led_c->led_amount=0;
led_c->led_curent_level=(led_bit)0;
...
void led_set_level(pLED_Class pthis,const int led_n,const led_bit led_level){//一位写
...
void led_read_level(pLED_Class pthis,const int led_n,led_bit *led_level){//一位写
...
void led_read_level_arr(pLED_Class pthis,led_bits* led_sta){//多位写
	for(int i=0;i < pthis -> led_amount;++i){
   *led_sta &= ~(0x01 << i);
